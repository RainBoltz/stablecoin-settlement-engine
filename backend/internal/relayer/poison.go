...
func (w *Worker) poison(ctx context.Context, d queue.Delivery, rep Report, v txfail.Verdict) (Report, error) {
	...
	if it.State != intent.StateSettling {
		return Report{Outcome: OutcomePoison,
			Detail: fmt.Sprintf("%s; job dropped, intent still %s", v.Reason, it.State)}, nil
	}
	...
	if sent != txseq.SentNo {
		...
		return Report{Outcome: OutcomePoison,
			Detail: fmt.Sprintf("%s; last broadcast %s, needs review", v.Reason, sent)}, nil
	}
	...
	return Report{Outcome: OutcomePoison, Detail: v.Reason + "; nothing was sent, so the intent failed"}, nil
}
